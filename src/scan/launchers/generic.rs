...
mod tests {
    ...
    #[test]
    fn fuzzy_matching_with_extra_tokens_or_punctuation() {
        let matcher = make_fuzzy_matcher();

        // Folder names may add release group suffixes or drop punctuation from the title.
        for (reference, candidate, matched) in [
            (
